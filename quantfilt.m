function y = quantfilt(x,n,p,blksz,DIM)
...
if ~isempty(DIM) && DIM > ndims(x)
	error(message('signal:quantfilt1:InvalidDimensions'))
end
...
%                       Local Function
%-------------------------------------------------------------------
function y = quantfilt1D(x,n,p,blksz)
...
for i=1:blksz:nx
    ...
    y(i:min(i+blksz-1,nx)) = quantile(xx,p,1);
end
